        ...
        area(content)
        {
            group(General)
            {
                ...
                field("Test ID"; Rec."Test ID")
                {
                    ...
                    begin
                        // If there's an existing No. Series, set it as the default selection
                        if NoSeries.Get(Rec."No. Series") then
                            NoSeriesPage.SetRecord(NoSeries);

                        // Run the No. Series page modally
                        if NoSeriesPage.RunModal() = Action::LookupOK then begin
                            // Get the selected No. Series record
                            NoSeriesPage.GetRecord(NoSeries);
                            Rec."No. Series" := NoSeries.Code;
                            Rec.Modify();
                        end;
                    end;
                }
